...
void sq::stats::hit( const std::string &idx, double taken ) {
    map[idx].push_back( taken );
}

std::vector<std::string> sq::stats::list() const {
...
double sq::stats::mini( const std::string &idx ) const {
    const auto &vec = map[idx];
    return *std::min_element(vec.begin(), vec.end());
}

double sq::stats::maxi( const std::string &idx ) const {
    const auto &vec = map[idx];
    return *std::max_element(vec.begin(), vec.end());
}

double sq::stats::total( const std::string &idx, double div ) const {
    long double t = 0;
    for( auto &in : map[idx] )
        t += in;
    return t / div;
...
double sq::stats::avg( const std::string &idx ) const {
    return total(idx, hits(idx) ? hits(idx) : 1 );
}

size_t sq::stats::hits( const std::string &idx ) const {
    return map[idx].size();
}

std::vector<std::string> sq::stats::report( const std::string &_fmt123456, const std::string &sort_key, bool reversed ) const {
